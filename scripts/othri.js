...
const model = {
    createItem(itemDesc, itemDateTime) {    
        ...
        if (localStorage.getItem(itemId) === null) { // make sure key doesn't already exist
            localStorage.setItem(itemId, `${itemDesc}@@@@@${itemDateTime}`);
        }
    ...
    readItems(descSearch, dateTimeSearch) { // [[id1, desc1, datetime1], [id2, desc2, datetime2]..]
        ...
        for (let i = 0; i < localStorageKeys.length; i++) {            
            ...
            if (localStorageKeys[i].startsWith('settings-')) continue; // don't display settings-type items, skip over them
    ...
    updateItem(currentItemId, itemDesc, itemDateTime) {
        ...
        if (newItemId != currentItemId) { // if date/time changed.. 
            if (localStorage.getItem(newItemId) === null) { // and new date/time/itemId doesn't already exist..
                this.deleteItem(currentItemId); // delete old item and..
                localStorage.setItem(newItemId, `${itemDesc}@@@@@${itemDateTime}`); // create new one
                return newItemId;
            }
        }
        else { // otherwise if date/time didn't change, just update existing item
            localStorage.setItem(currentItemId, `${itemDesc}@@@@@${itemDateTime}`); // create new one
